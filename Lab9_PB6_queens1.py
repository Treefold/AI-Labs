...
class Joc:
    ...
    def final(self, me):
        ...
        if Joc.SIMBOLURI_JUC[1][Joc.QUEEN] not in self.matr and Joc.SIMBOLURI_JUC[1][Joc.KING] not in self.matr:
            print ("WON", Joc.SIMBOLURI_JUC[0])
            return Joc.SIMBOLURI_JUC[0]
        if self.mutari(me, None, True):
...
def alpha_beta(alpha, beta, stare):
    ...
    try:
        ...
        print(stare)
        for i in stare.mutari_posibile:
            print (i)
...
def main():
    ...
    while not raspuns_valid:
        tip_algoritm='2'#input("Algorimul folosit? (raspundeti cu 1 sau 2)\n 1.Minimax\n 2.Alpha-beta\n ")
    ...
    raspuns_valid = False
    while not raspuns_valid:
        n = '4'#input("Adancime maxima a arborelui: ")
        if n.isdigit():
    ...
    raspuns_valid = False
    while not raspuns_valid:
        Joc.JMIN = '2'# input("Doriti sa jucati cu 1.'{}' sau cu 2.'{}'? ".format(s1[0], s2[0]))
        if (Joc.JMIN in ['1','2']):
            raspuns_valid = True
        else:
    ...
    while True :
        ...
        # stare_curenta.tabla_joc.fct_euristica(True)
