# 232 Surcea Mihai-Daniel Lab8 Pb2: Connect 4
...
class Joc:
    ...
    def final(self, afis = False, verifyAll = False):
        ...
        # Verificarea inainte sa citesc ca trebuie facuta doar ultima pozitie (ultima mutare) care nu exista in configuratia intiala este mai jos
        # (asa voiam sa fac de la inceput, doar ca am presupus ca nu am voie sa modific configuratia intiala <=> sa adaug pozitia ultimei mutari)
        # de asemenea, plurarul substantivelor m+a indus in eroare (linii, coloane, diagonale) <=> toate trebuie incercate
        if self.lastMovePoz == None: return False # no moves were made
        row, col = divmod(self.lastMovePoz, Joc.NR_COLOANE)
        # test row
        if afis: print("Row:",  self.matr[row*self.NR_COLOANE:(row+1)*self.NR_COLOANE])
        rez = TestFinal(self.matr[row*self.NR_COLOANE:(row+1)*self.NR_COLOANE])
        if rez != None: return rez
        # test column
        if row+1 >= Joc.NR_CONNECT: # <=> on this column there are enough pieces (to be relevant) to test it
            if afis: print("Col:", self.matr[col::self.NR_COLOANE])
            rez = TestFinal(self.matr[col::self.NR_COLOANE])
            if rez != None: return rez
        # test diag /
        diag = row + col
        if afis: print(f"Diag/ = {diag}")
        if diag + 1 >= Joc.NR_CONNECT and diag < Joc.NR_LINII+Joc.NR_COLOANE - Joc.NR_CONNECT: # <=> on this diag there can be enough pieces (to be relevant) to test it
            if diag < min (Joc.NR_LINII, Joc.NR_COLOANE): # in bottom triangle: 1
                if afis: print("Diag1 /:", self.matr[diag*Joc.NR_COLOANE:diag-1:-Joc.NR_COLOANE+1])
                rez = TestFinal(self.matr[diag*Joc.NR_COLOANE:diag-1:-Joc.NR_COLOANE+1])
                if rez != None: return rez
            elif diag >= max(Joc.NR_LINII, Joc.NR_COLOANE) - 1: # in top triangle: 3
                if afis: print("Diag3 /:", self.matr[max(diag, (diag-Joc.NR_COLOANE+2)*Joc.NR_COLOANE-1):(Joc.NR_LINII-1)*Joc.NR_COLOANE+diag+1:Joc.NR_COLOANE-1])
                rez = TestFinal(self.matr[max(diag, (diag-Joc.NR_COLOANE+2)*Joc.NR_COLOANE-1):(Joc.NR_LINII-1)*Joc.NR_COLOANE+diag+1:Joc.NR_COLOANE-1])
                if rez != None: return rez
            # in middle triangle: 2 
            elif Joc.NR_COLOANE > Joc.NR_LINII:
                if afis: print("Diag2 /:", self.matr[diag::Joc.NR_COLOANE-1])
                rez = TestFinal(self.matr[diag::Joc.NR_COLOANE-1])
                if rez != None: return rez
            else: # Joc.NR_LINII > Joc.NR_COLOANE
                if afis: print("Diag2 /:", self.matr[diag*Joc.NR_COLOANE:(diag-Joc.NR_COLOANE+1)*Joc.NR_COLOANE:-Joc.NR_COLOANE+1])
                rez = TestFinal(self.matr[diag*Joc.NR_COLOANE:(diag-Joc.NR_COLOANE+1)*Joc.NR_COLOANE:-Joc.NR_COLOANE+1])
                if rez != None: return rez
        # test diag \
        diag = row + Joc.NR_COLOANE - col - 1
        if afis: print(f"Diag\\ = {diag}")
        if diag + 1 >= Joc.NR_CONNECT and diag < Joc.NR_LINII+Joc.NR_COLOANE - Joc.NR_CONNECT: # <=> on this diag there can be enough pieces (to be relevant) to test it
            if diag < min (Joc.NR_LINII, Joc.NR_COLOANE): # in bottom triangle: 1
                if afis: print("Diag1 \\:", self.matr[Joc.NR_COLOANE-diag-1:(Joc.NR_CONNECT+diag-2)*Joc.NR_COLOANE:Joc.NR_COLOANE+1])
                rez = TestFinal(self.matr[Joc.NR_COLOANE-diag-1:(Joc.NR_CONNECT+diag-2)*Joc.NR_COLOANE:Joc.NR_COLOANE+1])
                if rez != None: return rez
            elif diag >= max(Joc.NR_LINII, Joc.NR_COLOANE) - 1: # in top triangle: 3
                if afis: print("Diag3 \\:", self.matr[(diag-Joc.NR_COLOANE+1)*Joc.NR_COLOANE::Joc.NR_COLOANE+1])
                rez = TestFinal(self.matr[(diag-Joc.NR_COLOANE+1)*Joc.NR_COLOANE::Joc.NR_COLOANE+1])
                if rez != None: return rez
            # in middle triangle: 2 
            elif Joc.NR_COLOANE > Joc.NR_LINII:
                if afis: print("Diag2 \\:", self.matr[Joc.NR_LINII*(Joc.NR_COLOANE+1)-diag-2::-Joc.NR_COLOANE-1])
                rez = self.matr[Joc.NR_LINII*(Joc.NR_COLOANE+1)-diag-2::-Joc.NR_COLOANE-1]
                if rez != None: return rez
            else: # Joc.NR_LINII > Joc.NR_COLOANE
                if afis: print("Diag2 \\:", self.matr[(diag-Joc.NR_COLOANE+1)*Joc.NR_COLOANE:(diag+1)*Joc.NR_COLOANE:Joc.NR_COLOANE-1])
                rez = self.matr[(diag-Joc.NR_COLOANE+1)*Joc.NR_COLOANE:(diag+1)*Joc.NR_COLOANE:Joc.NR_COLOANE-1]
                if rez != None: return rez
        
        if not verifyAll: return False # do not verify all pieces if it's not necesarry
        print ("Start to verify all")
    
        # Verificarea inainte sa citesc ca trebuie facuta doar ultima pozitie (ultima mutare) care nu exista in configuratia intiala 
        ...
        for i in range (Joc.NR_COLOANE-Joc.NR_LINII-1):
            ...
            if rez != None: return rez
        # top triangle: 3
        # when Row == COL the diagonal must be counted only once
...
def afis_daca_final(stare_curenta):
    ...
    final = stare_curenta.tabla_joc.final() #True) # Make it true for debugging
    if(final):
        if (final=="remiza"):
            print("Remiza!")
        else:
            ...
            while not raspuns_valid:
                try:
                    ...
                    try:
                        ...
                        stare_curenta.tabla_joc.lastMovePoz = linie * Joc.NR_COLOANE + coloana
                        raspuns_valid = True
                    except ValueError: 
...
# win sequence for the 0: 
# 2 (AB)
# 6 (adancime maxima)
# 0 (jucatorul meu) 
# Ordinea mutarilor: 0 2 3 1 1 1 5 0 5 6 5 4 6 0 6 6 2