...
class Joc:
	...
	NR_COLOANE = 6
	TO_CONNECT = 4
	# assert TO_CONNECT <= NR_LINII and TO_CONNECT <= NR_COLOANE
	JMIN = None
	JMAX = None
	GOL = '#'
	def __init__(self, tabla=None):
		self.mat = tabla or [Joc.GOL] * NR_LINII * NR_COLOANE
		...
		for row in range (self.NR_LINII):
			...
			for x in sele.mat[row]:
				if x == self.GOL:
					...
					leng = 1
					last = x
				if leng == self.TO_CONNECT: return last # someone won
	...
	def mutari_joc(self, jucator):
		"""
		Pentru configuratia curenta de joc "self.mat" (de tip lista, cu 9 elemente),
		...
		for col in range (self.NR_COLOANE):
			try:
				poz = self.mat[col::self.NR_COLOANE].index(self.GOL)
				poz = poz * self.NR_COLOANE + col
				mutare = deepcopy(self.mat)
				mutare[poz] = jucator
				l_mutari.append (Joc(mutare))
			except ValueError:
				continue # this column is full (cannot place a piece here)
		return l_mutari
	...
	def linii_deschise(self, jucator):
		return (self.linie_deschisa(self.mat[0:3],jucator) 
			+ self.linie_deschisa(self.mat[3:6], jucator) 
			+ self.linie_deschisa(self.mat[6:9], jucator)
			+ self.linie_deschisa(self.mat[0:9:3], jucator)
			+ self.linie_deschisa(self.mat[1:9:3], jucator)
			+ self.linie_deschisa(self.mat[2:9:3], jucator)
			+ self.linie_deschisa(self.mat[0:9:4], jucator) #prima diagonala
			+ self.linie_deschisa(self.mat[2:8:2], jucator)) # a doua diagonala
	...
	def __str__(self):
		sir= (" ".join([str(x) for x in self.mat[0:3]])+"\n"+
		" ".join([str(x) for x in self.mat[3:6]])+"\n"+
		" ".join([str(x) for x in self.mat[6:9]])+"\n")
...
class Stare:
	...
	def __init__(self, tabla_joc, j_curent, adancime, parinte=None, scor=None):
		self.tabla_joc = tabla_joc  # un obiect de tip Joc => „tabla_joc.mat”
...
def main():
	...
	while True :
		...
		#muta jucatorul
			...
			while not raspuns_valid:
				try:
					...
					if (linie in range(0,3) and coloana in range(0,3)):
						if stare_curenta.tabla_joc.mat[linie*3+coloana] == Joc.GOL:					
							raspuns_valid=True
						else:
			...
			stare_curenta.tabla_joc.mat[linie*3+coloana]=Joc.JMIN
