...
def main():
    ...
    raspuns_valid = False
    while not raspuns_valid:
        n = int(input("Vrei sa folosesti interfata grafica:1.Da sau 0.Nu"))
        if n in [0,1]:
            gui = (n == 1)
            raspuns_valid = True
        else:
        ...
        while True:
            try:
                ...
                if rsp < mi or rsp > len(pos):
                    print(f'Invalid choice (choose between {mi} and {len(pos)} or q).')
                    continue 
                print ("Choice:", rsp)
                return rsp      
            except ValueError:
    ...
    while not lost:
        if (stare_curenta.j_curent[0] == Joc.JMIN[0]):
            ...
            if gui:
                patratele=deseneaza_grid(ecran,stare_curenta.tabla_joc.matr, pos)
            ...
            while True:
                ...
                if gui:
                    patratele=deseneaza_grid(ecran,stare_curenta.tabla_joc.matr, pos, src)
                    rsp = None
                    while rsp == None:
                        for event in pygame.event.get():
                            if event.type== pygame.QUIT:
